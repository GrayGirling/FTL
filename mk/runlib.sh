...
# Run the microsoft 'lib' static library archiver giving the appropriate
...
if [ -z "$VC_PATH" ]; then
    ...
    # we have to have a fully qualified path name to lib because when we run it
    # we will have changed the "PATH" environment variable to a Windows path
    cmd_lib=`PATH="$PATH:$vcpath" which lib`
    PATH="$VC_PATH" INCLUDE="$VC_INCLUDE" LIB="$VC_LIB" "$cmd_lib" "$@"
    rc=$?
else
