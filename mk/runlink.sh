...
# Run the microsoft 'link' linker giving the appropriate arguments on the
# command line
...
if [ -z "$VC_PATH" ]; then
    ...
    # we have to have a fully qualified path name to link because when we run it
    # we will have changed the "PATH" environment variable to a Windows path
    cmd_link=`PATH="$vcpath:$PATH" which link`
    PATH="$VC_PATH" INCLUDE="$VC_INCLUDE" LIB="$VC_LIB" "$cmd_link" "$@"
    rc=$?
else
